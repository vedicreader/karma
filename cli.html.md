...
The flags are `context`’s flags, and they mean the same thing here. They
used to be one `--sync_graph` that defaulted to off, so this command
promised a call graph in its own help text and never built one —
`kosha ni` then found nothing and there was no way to tell why.
