...
``` python
def sync(
    dir:str | None=None, pkgs:list | None=None, embed:bool=True, force:bool=False, repo:bool=True, env:bool=True,
    graph:bool=True
)->dict:
```
...
`env=False` is ‘this repo only’; `graph=False` skips the call graph,
which `ni` and `where_to_add` need. Returns post-sync status.*
