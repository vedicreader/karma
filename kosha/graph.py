...
@patch
def dep_stack(self:Kosha, seeds:list=None, depth:int=1) -> list:
...
@patch
def task_context(self:Kosha, q:str, depth:int=3, limit:int=10) -> dict:
	...
	repo_pkgs = L(self.code_rd(select='distinct to_pkg')).attrgot('to_pkg').filter(spec)
	seeds = list(dict.fromkeys(list(pkgs) + [p for p in repo_pkgs if p not in set(pkgs)]))
	layers = self.dep_stack(seeds, depth=depth) if seeds else []
