...
# %% auto #0
__all__ = ['mcp', 'status', 'sync', 'context', 'repo_context', 'env_context', 'where_to_add', 'similar', 'peers', 'code_clusters',
           'node_info', 'neighbors', 'short_path', 'public_api', 'top_nodes', 'api_paths', 'dep_stack', 'pkg_url',
           'main']
...
mcp = MCPServer('kosha', instructions=(
    ...
    'points; similar/peers start from a path:line instead of a query, and code_clusters maps the repo. '
...
def similar(path:str, line:int=1, limit:int=15, graph:bool=False) -> list:
    ...
    return _jsonable(_kosha().similar(path, line=line, limit=limit, graph=graph))

@mcp.tool()
def peers(path:str, line:int=1, limit:int=25, graph:bool=False) -> dict:
    "The cluster the chunk at path:line belongs to — its family, not a ranked list. Ask this instead of similar when the question is 'where else did we already do this?'. Check `note`: it names the backend, or says why the answer degraded to k-NN."
    return _jsonable(_kosha().peers(path, line=line, limit=limit, graph=graph))

@mcp.tool()
def code_clusters(limit:int=20, min_size:int=2) -> dict:
    "A map of the indexed repo by shape: labelled clusters with the files each spans. Use to orient in an unfamiliar codebase before searching it."
    return _jsonable(_kosha().code_clusters(limit=limit, min_size=min_size))
